...
export default function Table({ dataTh, dataTd, coloumDB, buttonData, endpoints, coloumnDetail, judulModalEdit, inputData }) {
                ...
                <div className="flex justify-center mb-5 space-x-2" text="center">
    {
        buttonData.includes("create") ? (
            <button onClick={handleModalAdd} type="button" className="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-900 border-green-800 bg-green-800 hover:bg-green-100 hover:text-green-100 focus:outline-none focus:ring-4 dark:bg-green-800 bg-green-700 dark:text-gray-400 dark:border-gray-600 dark:hover:bg-green-700 dark:hover:text-white dark:focus:ring-gray-700">Create</button>
        ) : ''
    }
    {
        buttonData.includes("trash") ? (
            <Link to={"/stuff/trash"} className="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-900 bg-yellow-800 border-yellow-800 rounded-lg hover:bg-yellow-100 hover:text-yellow-700 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:bg-yellow-800 dark:text-gray-400 dark:border-gray-600 dark:hover:bg-yellow-700 dark:hover:text-white dark:focus:ring-gray-700">Trash</Link>
        ) : ''
    }
                </div>
                    ...
                    <tbody>
                        {
                            Object.entries(dataTd).map(([index, value]) => (
                                <tr
                                    class="border-gray-800 dark:bg-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-600">
